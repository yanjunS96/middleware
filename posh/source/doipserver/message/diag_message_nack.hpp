#ifndef DIAG_MESSAGE_NACK_HPP
#define DIAG_MESSAGE_NACK_HPP
...
namespace motovis
{
    namespace doip
    {
        ...
        protected:
            void SetPayloadLen(std::vector<uint8_t> &payload) const override
            {
                std::size_t vecLen{cHeaderSize
                                   + sizeof mSourceAddress + sizeof mTargetAddress + sizeof mNackCode};
            ...
            void GetPayload(std::vector<uint8_t> &payload) const override
            {
                ...
                memmove(&payload[offSize],&mNackCode,std::min(
                        (int)(payload.size() - offSize), (int)(sizeof mNackCode)));
                offSize += sizeof mNackCode;
                if (mHasPreviousMessage)
                {
        ...
        public:
            DiagMessageNack(): Message(cPayloadType){}
...
//        constexpr PayloadType DiagMessageNack::cPayloadType;
    }
}
